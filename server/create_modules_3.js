...
// Other routes stubs
const otherModules = ['academic-years', 'classes', 'courses', 'subjects', 'topics', 'enrollments', 'teacher-assignments', 'lessons', 'materials', 'dashboard', 'config'];
otherModules.forEach(m => {
  write(path.join(serverDir, 'modules', m, m + '.routes.ts'), `
import { Router } from 'express';
const router = Router();
export default router;
  `);
  write(path.join(serverDir, 'modules', m, m + '.controller.ts'), `
import { Request, Response, NextFunction } from 'express';
export class Controller {}
export const controller = new Controller();
  `);
  write(path.join(serverDir, 'modules', m, m + '.service.ts'), `
export class Service {}
export const service = new Service();
  `);
  write(path.join(serverDir, 'modules', m, m + '.validation.ts'), `
import { z } from 'zod';
